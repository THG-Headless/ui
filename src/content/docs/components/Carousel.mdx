---
title: Carousel
description: Carousel
---

import ShowComponent from "@components/doc-helpers/ShowComponent.astro";
import BasicCarousel from "@components/carousel/BasicCarousel.html";
import ProductRail_withBullets from "@components/carousel/ProductRail_withBullets.html";
import ProductRail_withBulletsAndControls from "@components/carousel/ProductRail_withBulletsAndControls.html";
import ProductRail_Additional from "@components/carousel/ProductRail_Additional.html";
import CarouselBanner from "@components/carousel/CarouselBanner.html";
import CarouselControls from "@components/carousel/CarouselControls.html";
import CarouselControls_ProductRail from "@components/carousel/CarouselControls_ProductRail.html";
import Bullets from "@components/carousel/CarouselBullets.html";

Used to display lists of cards in a horizontally scrollable area.

## Slides

Swipe across to see remaining slides.

<ShowComponent filePath="./src/components/carousel/BasicCarousel.html">
  <BasicCarousel />
</ShowComponent>

## Bullets

Indicate the remaining unseen slides. Use the `carousel-bullet-hover` and `carousel-bullet-active` modifiers to style the hover and active states. These will need to be applied and removed with JavaScript.

<ShowComponent filePath="./src/components/carousel/CarouselBullets.html">
  <Bullets />
</ShowComponent>

## Controls

Use these to navigate between slides.

<ShowComponent filePath="./src/components/carousel/CarouselControls.html">
  <CarouselControls />
</ShowComponent>

They use the`btn` class (see [Button](/components/button/)), and additional styling is applied by the `carousel-control-btn` modifier class.

## Examples

### Hero banner

Use for full-width slides.

<ShowComponent filePath="./src/components/carousel/CarouselBanner.html">
  <CarouselBanner />
</ShowComponent>

### Product Rail

<ShowComponent filePath="./src/components/carousel/ProductRail_withBulletsAndControls.html">
  <ProductRail_withBulletsAndControls />
</ShowComponent>

### Product Rail w/ Additional Controls

<ShowComponent filePath="./src/components/carousel/ProductRail_withBulletsAndControls.html">
  <ProductRail_Additional />
</ShowComponent>

### Variables

The carousel can be customized with the following variables:

| Variable                       | default                |
| ------------------------------ | ---------------------- |
| `--carousel-width`             | `100%`                 |
| `--carousel-item-gap`          | `1rem`                 |
| `--carousel-dummy-image-color` | `var(--secondary-300)` |
| `--carousel-bullet-color`      | `var(--neutral-300)`   |

## Implementation Notes

When implementing a carousel:

### A11y

- [ ] The CSS-only version is not WCAG 2.1 compliant by default.
- [ ] Make sure to add the correct [aria labels](https://www.w3.org/WAI/ARIA/apg/patterns/carousel/#wai-ariaroles,states,andproperties) to make the component accessible: I.e
- [ ] Ensure that aria labels for slides are correct.
- [ ] The carousel container has the aria-roledescription property set to carousel.
- [ ] Make sure each slide has an accessible name
- [ ] Make sure each slide container has role group with the property aria-roledescription set to slide.

### Interactivity

- It's recommended to use the [Intersection Observer API](https://developer.mozilla.org/en-US/docs/Web/API/Intersection_Observer_API) to add interactivity.

## Further reading

Carousel Pattern on W3C Web Accessibility Initiative
https://www.w3.org/WAI/ARIA/apg/patterns/carousel/

W3.org carousel tutorial
https://www.w3.org/WAI/tutorials/carousels/

CSS Only Carousel CSS-Tricks
https://css-tricks.com/css-only-carousel/
