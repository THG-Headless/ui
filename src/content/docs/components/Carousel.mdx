...
| `--carousel-item-gap`                   | `1rem`        | Gap between carousel items (default size)        |
| `--carousel-content-padding`            | `0rem`        | Padding around content (default size)            |
| `--carousel-content-gap`                | `0.5rem`      | Gap between content elements (default size)      |
| `--carousel-image-height`               | `300px`       | Height of carousel images (default size)         |
| `--carousel-image-width`                | `300px`       | Width of carousel images (default size)          |
| `--carousel-banner-image-width`         | `50%`         | Width of banner images (default size)            |
| `--carousel-banner-image-height`        | `400px`       | Height of banner images (default size)           |
| `--carousel-banner-image-height-mobile` | `300px`       | Height of banner images on mobile (default size) |
...
| `--carousel-small-image-width`                | `200px`       | Width of carousel images (small size)          |
...
| `--carousel-large-image-width`                | `400px`       | Width of carousel images (large size)          |
...
| Variable                          | Default Value                    | Description                                  |
| --------------------------------- | -------------------------------- | -------------------------------------------- |
| `--carousel-surface-image`        | `var(--surface-inactive)`        | Background color for carousel images         |
| `--carousel-surface-item`         | `var(--surface-transparent)`     | Background color for carousel items          |
| `--carousel-surface-item-hover`   | `var(--carousel-surface-item)`   | Background color for carousel items on hover |
| `--carousel-surface-banner`       | `var(--surface-white)`           | Background color for banner items            |
| `--carousel-surface-banner-hover` | `var(--carousel-surface-banner)` | Background color for banner items on hover   |
| `--carousel-border-hover`         | `var(--border-primary)`          | Border color on hover                        |
| `--carousel-border-active`        | `var(--border-primary)`          | Border color when active                     |
| `--carousel-border-width-image`   | `0px`                            | Border width for images                      |
| `--carousel-focus-outline`        | `var(--border-attention)`        | Outline color when focused                   |

### Text Variables

| Variable                            | Default Value                  | Description               |
| ----------------------------------- | ------------------------------ | ------------------------- |
| `--carousel-text-color`             | `var(--text-black)`            | Default text color        |
| `--carousel-text-title-color`       | `var(--text-high-emphasis)`    | Color for titles          |
| `--carousel-text-body-color`        | `var(--text-medium-emphasis)`  | Color for body text       |
| `--carousel-text-title-size`        | `var(--text-size-md)`          | Font size for titles      |
| `--carousel-text-title-weight`      | `var(--font-weight-semi-bold)` | Font weight for titles    |
| `--carousel-text-title-line-height` | `var(--line-height-md)`        | Line height for titles    |
| `--carousel-text-body-size`         | `var(--text-size-body)`        | Font size for body text   |
| `--carousel-text-body-weight`       | `var(--font-weight-normal)`    | Font weight for body text |
...
| `--carousel-radius-image`         | `var(--radius-size-none)`        | Border radius for images      |
| `--carousel-animation-speed`      | `var(--animation-speed-default)` | Speed of animations           |
...
Some interactivity features should be added using Javascript.
