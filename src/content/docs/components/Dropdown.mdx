...
<div style={{ marginBottom: "200px" }}>
  <ShowComponent filePath="./src/components/dropdown/BasicDropdownOpen.html">
    <BasicDropdownOpen />
  </ShowComponent>
</div>
...
<div style={{ marginBottom: "240px" }}>
  <ShowComponent filePath="./src/components/dropdown/SearchDropdownOpen.html">
    <SearchDropdownOpen />
  </ShowComponent>
</div>
...
- [x] The dropdown wrapper uses the `combobox` role with appropriate ARIA attributes
...
- [x] Summary element is properly labeled with `aria-labelledby`
- [x] Options marked with `role="option"` and `aria-selected` states
- Expanded state indicated with `aria-expanded` on the button and combobox wrapper
