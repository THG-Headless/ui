...
| Variable | Default Value | Description |
|----------|---------------|-------------|
| `--acc-icon-expanded-transform` | `0px` | Transformation to apply when panel is expanded |
| `--acc-icon-expanded-transition` | `1em 1.5em` | Transition to apply when panel is expanded |
| `--acc-icon-collapsed-transform` | `0px` | Transformation to apply when panel is collapsed |
| `--acc-icon-collapsed-transition` | `1em 1.5em` | Transition to apply when panel is collapsed |
...
#### ARIA State Management (Basic Accordion Only)
...
If using the Summary/Details HTML elements, the accessible name is inferred automatically from the contents of these elements. There remain inconsistencies and gaps in assitive technologies ability to perceive this which is why we do not yet promote this format as an accessible option.
